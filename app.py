...
def func2():
    ...
    return render_template("play.html",thumbnailList=data)
...
def upload():
    if request.method=="POST":
        ...
        conn=sqlite3.connect("youtube.db")
        cur=conn.cursor()
        cur.execute("insert into thumbnailList (THUMBNAIL,PROFILEPIC,DESCRIPTION,UPLOADED_BEFORE,CHANNEL,VIEWS) values(?,?,?,?,?,?)",(thumbnail,profile,description,uploadInfo,channel,views))
        conn.commit()
        return redirect("/")
